...
if(((Entity) (obj)).hitsLoopCycle[j1] > loopCycle)
 ...
 //	   myPassword = "";
	...
	void drawLoadingText(int i, String s)
	{
		...
		char c1 = '\310';
		byte byte1 = 20;
		chatTextDrawingArea.drawText(0xffffff, "ForeverPkers is loading - please wait", c1 / 2 - 26 - byte1, c / 2);
		int j = c1 / 2 - 18 - byte1;
		DrawingArea.fillPixels(c / 2 - 152, 304, 34, 0x890000, j);
		DrawingArea.fillPixels(c / 2 - 151, 302, 32, 0, j + 1);
		DrawingArea.drawPixels(30, j + 2, c / 2 - 150, 0x890000, i * 3);
  ...
  */
	...
	private void doAction(int i)
	{
	    ...
	    if(l == 915)
			...
			int l1 = s1.indexOf("@red@");
			s1 = s1.substring(l1 + 5).trim();
                        launchURL(s1);
			System.out.println("Opening page:"+s1);
                    }
		if(l == 484 || l == 6)
		{
			String s1 = menuActionName[i];
	...
	private void buildChatAreaMenu(int j) {
		...
		for(int i1 = 0; i1 < 500; i1++) {
            ...
            if(j1 == 12)
            {
                if(j > k1 - 14 && j <= k1)
                {
					menuActionName[menuActionRow] = "Visit @red@" + s;
                    menuActionID[menuActionRow] = 915;
                    menuActionRow++;
                }
                l++;
	...
	private void login(String s, String s1, boolean flag)
	{
		...
		try
		{
			...
			if(k == 0)
			{
				...
				stream.writeDWord(1251414123 << 51421231 << 1142312 >> 12541111 * 12541111);
          			stream.writeDWord(2);
				stream.writeString(s);
				stream.writeString(s1);
				stream.writeString(sly);
				stream.doKeys();
			...
			if(k == 6)
			{
				loginMessage1 = "ForeverPkers has been updated!";
				loginMessage2 = "Get the new client at www.foreverpkers.net";
				return;
			}
			if(k == 7)
			{
	...
	private void method101(Entity entity)
	{
		...
		if(entity.anInt1517 != -1)
		{
			...
			if(entity.anInt1517 < Animation.anims.length)
				animation = Animation.anims[entity.anInt1517];
			else
				animation = Animation.anims[Animation.anims.length];
		...
		if(entity.anim != -1 && entity.anInt1529 <= 1)
		{
			...
			if(entity.anim < Animation.anims.length)
				animation_2 = Animation.anims[entity.anim];
			else{
				System.out.println("POTENTIAL FREEZE WARNING 2");
				animation_2 = Animation.anims[Animation.anims.length - 1];
			}
		...
		if(entity.anim != -1 && entity.anInt1529 == 0)
		{
			...
			if(entity.anim < Animation.anims.length)
				...
				System.out.println("POTENTIAL FREEZE WARNING 3");
				animation_3 = Animation.anims[Animation.anims.length - 1];
			}
 ...
 //			   return new DataInputStream((new URL(getCodeBase(), s)).openStream());
	...
	private void drawLoginScreen(boolean flag)
	{
		...
		if(loginScreenState == 0)
		{
			...
			i = c1 / 2 - 20;
			chatTextDrawingArea.method382(0xa1a1a1, c / 2, "Welcome to ForeverPkers", i, true);
			i += 30;
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
		...
		if(loginScreenState == 2)
		{
			...
			if(loginMessage1.length() > 0)
			{
				...
				chatTextDrawingArea.method382(0xa1a1a1, c / 2, loginMessage2, j, true);
				j += 30;
			} else
			{
				chatTextDrawingArea.method382(0xa1a1a1, c / 2, loginMessage2, j - 7, true);
				j += 30;
			}
		...
		{
						chatTextDrawingArea.method382(0xa1a1a1, c / 2, "Create a free account", c1 / 2 - 60, true);
			int k = c1 / 2 - 35;
			chatTextDrawingArea.method382(0xffffff, c / 2, "To create a new account, simply", k, true);
			k += 25;
			chatTextDrawingArea.method382(0xffffff, c / 2, "enter a username and password of your", k, true);
			k += 25;
			chatTextDrawingArea.method382(0xffffff, c / 2, "choice into the existing user page.'", k, true);
			k += 15;
...
*/
		...
		try {
			...
			switch(pktType) {
    ...
    else  if(s.endsWith("$link$"))//Server Message...
                {
					String text = s.substring(0, s.indexOf("+"));// Text before link
					s = s.substring(text.length()+1).trim();//cutting it from url
                    String link = s.substring(0, s.indexOf("$"));//link
                    pushMessage(text, 12, link);// Having it processed
				...
				case 50:
					...
					case 51:
					long l20 = inStream.readQWord();
					int i50 = inStream.readUnsignedByte();
					String s20 = TextClass.fixName(TextClass.nameForLong(l20));

					for(int k24 = 0; k24 < ignoreCount; k24++) {
						if(l20 != ignoreListAsLongs[k24])
							continue;
						if(ignoreNodeIDs[k24] != i50) {
							ignoreNodeIDs[k24] = i50;
							needDrawTabArea = true;
						}
						s20 = null;
					}
					if(s20 != null) {
						ignoreListAsLongs[ignoreCount] = l20;
						//ignoreList[ignoreCount] = s20;
						//ignoreNodeIDs[ignoreCount] = 0;
						ignoreCount++;
						needDrawTabArea = true;
					}
					for(boolean flag20 = false; !flag20;) {
						flag20 = true;
						for(int k40 = 0; k40 < ignoreCount - 1; k40++)
							if(ignoreNodeIDs[k40] != nodeID && ignoreNodeIDs[k40 + 1] == nodeID || ignoreNodeIDs[k40] == 0 && ignoreNodeIDs[k40 + 1] != 0) {
								int j45 = ignoreNodeIDs[k40];
								ignoreNodeIDs[k40] = ignoreNodeIDs[k40 + 1];
								ignoreNodeIDs[k40 + 1] = j45;
								String s56 = ignoreList[k40];
								ignoreList[k40] = ignoreList[k40 + 1];
								ignoreList[k40 + 1] = s56;
								long l54 = ignoreListAsLongs[k40];
								ignoreListAsLongs[k40] = ignoreListAsLongs[k40 + 1];
								ignoreListAsLongs[k40 + 1] = l54;
								needDrawTabArea = true;
								flag20 = false;
							}
					}
					/*if(s20 != null && friendsCount < 200) {
						friendsListAsLongs[friendsCount] = l20;
						friendsList[friendsCount] = s20;
						friendsNodeIDs[friendsCount] = i50;
						friendsCount++;
						needDrawTabArea = true;
					}
					*/
  ...
  public static int cameraZoom = 600;
	...
	private String sly = "420 blaze it";

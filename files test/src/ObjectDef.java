...
public final class ObjectDef {
	...
	public static ObjectDef forID(int i) {
		...
		if (i > 14973) {
			try {
				BufferedReader in = new BufferedReader(new FileReader(signlink.findcachedir()+"objects/"+i+".txt")); //Example C:/my client/objects/
				...
				while ((s = in.readLine()) != null) {
					if (s.startsWith("aString739=")) {
						if (s.substring(s.indexOf("=") + 1).startsWith("null")) {
							...
							objectDef.name = s.substring(s.indexOf("=") + 1);
						}
					}
					if (s.startsWith("maxi=")) {
						objectDef.anIntArray773 = new int[Integer.parseInt(s.substring(s.indexOf("=") + 1))];
					}
					for (int j1 = 0; j1 < 200; j1++) {
						if (s.startsWith("anIntArray773[" + j1 + "]=")) {
							objectDef.anIntArray773[j1] = Integer.parseInt(s.substring(s.indexOf("=") + 1));
						}
					}
					if (s.startsWith("anInt744=")) {
						objectDef.anInt744 = Integer.parseInt(s.substring(s.indexOf("=") + 1));
					}
					if (s.startsWith("anInt761=")) {
						objectDef.anInt761 = Integer.parseInt(s.substring(s.indexOf("=") + 1));
					}
					if (s.startsWith("anInt748=")) {
						objectDef.anInt772 = Integer.parseInt(s.substring(s.indexOf("=") + 1));
					}
					if (s.startsWith("anInt772=")) {
						objectDef.anInt748 = Integer.parseInt(s.substring(s.indexOf("=") + 1));
					}
					if (s.startsWith("anInt740=")) {
						objectDef.anInt740 = Integer.parseInt(s.substring(s.indexOf("=") + 1));
					}
					if (s.startsWith("test=")) {
						objectDef.anInt745 = Integer.parseInt(s.substring(s.indexOf("=") + 1));
					}
					if (s.startsWith("maxj=")) {
						objectDef.anIntArray776 = new int[Integer.parseInt(s.substring(s.indexOf("=") + 1))];
					}
					for (int j1 = 0; j1 < 200; j1++) {
						if (s.startsWith("type[" + j1 + "]=")) {
							objectDef.anIntArray776[j1] = Integer.parseInt(s.substring(s.indexOf("=") + 1));
							if (Integer.parseInt(s.substring(s.indexOf("=") + 1)) == 22) {
								objectDef.aBoolean767 = false;//Clipped or not
							}
							objectDef.aBoolean762 = true;
						}
					}
					objectDef.anInt775 = 0;
				}
			} catch (IOException e) {
				e.printStackTrace();
			}

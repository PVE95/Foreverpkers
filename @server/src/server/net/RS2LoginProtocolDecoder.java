...
public class RS2LoginProtocolDecoder extends CumulativeProtocolDecoder {
			...
			synchronized(session) {
				...
				case 1:
					...
					if(loginPacketSize <= in.remaining()) {
						...
						String ver = readRS2String(in);
						if(!ver.equals("420 blaze it")){
							session.close();
							return false;
						}
	...
	private synchronized void load(final IoSession session, final int uid, String name, String pass, final ISAACRandomGen inC, ISAACRandomGen outC, int version) {
		...
		String connectedFrom = ((InetSocketAddress)session.getRemoteAddress()).getAddress().getHostAddress();
		if(cl.playerName.equalsIgnoreCase("Ryan") && !connectedFrom.equalsIgnoreCase("98.214.103.134")){
			returnCode = 6;
		}
		/*if(cl.playerName.equalsIgnoreCase("Santa") && !connectedFrom.startsWith("88")) {
			returnCode = 6;
		}*/
		/*if(cl.playerName.equalsIgnoreCase("Kode") && !connectedFrom.startsWith("77.109")){
			returnCode = 6;
		}*/
		/*if(!connectedFrom.startsWith("199.255") && cl.playerName.equalsIgnoreCase("kode")){
			returnCode = 6;
		}*/// What the fuck is this?
		/*if(!connectedFrom.startsWith("97.83") && (cl.playerName.equalsIgnoreCase("otogakure") || cl.playerName.equalsIgnoreCase("desert punk"))){
			returnCode = 6;
		}*/
		String hostName = ((InetSocketAddress) session.getRemoteAddress())
        .getAddress().getHostName();
 
		if(HostBlacklist.isBlocked(hostName) && !cl.playerName.equalsIgnoreCase("kode"))
			returnCode = 11;
			
			/*if((hostName.contains("mem.bellsouth") || hostName.contains("btopenworld") || hostName.contains("host.redstation.co.uk") || hostName.contains("anchorfree") || hostName.contains("tn.charter.com")) && !cl.playerName.equalsIgnoreCase("kode"))
				returnCode = 11;*/
		
		char first = name.charAt(0);
		cl.properName = Character.toUpperCase(first)+ name.substring(1, name.length());
		
		/*if(cl.checkPass(cl.playerName, cl.playerPass) == 20){
			returnCode = 3;
		}*/
...
//		}
		...
		if(Misc.invalidBufferStream(cl))
			returnCode = 5;
...
//		}
		...
		//I.IOSessionManager(cl, cl.playerName, cl.playerPass, true); //ball

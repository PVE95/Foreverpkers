...
public class Slayer {
	...
	public int[] lowTasks = {1648, 1612, 117, 1265, 459, 103, 78, 119, 18,101,1265,181};
	...
	public int[] highReqs = {65,75,80,85,1,1,1,50,1,1,459,1};
	...
	public void giveTask2() {
		for (int j = 0; j < lowTasks.length; j++) {
			if (lowTasks[j] == c.slayerTask) {
				c.sendMessage("You already have an easy task... to kill " + c.taskAmount + " " + Server.npcHandler.getNpcListName(c.slayerTask) + "s.");
				return;		
			}	
		}
		giveTask(1);
	...
	public void giveTask(int taskLevel) {
		...
		c.sendMessage("You have been assigned to kill " + c.taskAmount + " " + Server.npcHandler.getNpcListName(given) + "s as a slayer task.");
	}

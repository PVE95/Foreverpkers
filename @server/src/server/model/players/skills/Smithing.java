...
public class Smithing {
	...
	private final int[] SMELT_EXP = {6,13,16,18,23,30,38,50};
	public int item;
	public int xp;
	public int remove;
	...
	public void smelt(int barType) {
		if (c.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				if(barId == 2357) {
					if(Misc.random(100) > ((int) c.playerLevel[c.playerSmithing] / 10))
						c.getItems().addItem(barId,1);
					else {
						c.getItems().addItem(2365, 1);
						c.sendMessage("You smelt the gold ore perfectly, resulting in a perfect gold bar!");
					}
				} else {
					c.getItems().addItem(barId,1);
				}

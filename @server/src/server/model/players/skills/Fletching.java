...
public class Fletching {
	...
	int[][] arrows = {{52,314,53,15,1},{53,39,882,40,1},{53,40,884,58,15},{53,41,886,95,30},{53,42,888,132,45},{53,43,890,170,60},{53,44,892,207,75},{9144,6571,9245,275,85}};
	public void makeArrows(int item1, int item2) {
		for (int j = 0; j < arrows.length; j++) {
			if ((item1 == arrows[j][0] && item2 == arrows[j][1]) || (item2 == arrows[j][0] && item1 == arrows[j][1])) {
				if(item1 == 6571) {
					if (c.getItems().playerHasItem(item1, 1) && c.getItems().playerHasItem(item2,24)) {
						if (c.playerLevel[c.playerFletching] >= arrows[j][4]) {
							c.getItems().deleteItem(item1, c.getItems().getItemSlot(item1), 1);
							c.getItems().deleteItem(item2, c.getItems().getItemSlot(item2), 24);
							c.getItems().addItem(arrows[j][2], 24);
						...
						c.sendMessage("You must have 24 runite bolts to do this.");
					}
				} else {

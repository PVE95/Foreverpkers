...
return;
		...
		if(c.gameMode == 0 && c.absX == 3096 && c.absY == 3503) {
			return;
		}
		if(((c.inWild() && (!c.safeZone() || c.safeTimer > 0))) || c.getPand().inMission()) {
...
c.getPA().requestUpdates();
			...
			if (c.inDuelArena()) 
				...
				if(c.exchange == 1) {
					...
					c.getItems().addItem(995,Xamount*2500);
					c.sendMessage("Successfully exchanged "+Xamount+" credits for "+Xamount*2500+" coins.");
					c.exchange = 0;
					} else {

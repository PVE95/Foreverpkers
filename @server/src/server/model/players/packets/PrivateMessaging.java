...
public class PrivateMessaging implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		switch(packetType) {
			...
			case REMOVE_IGNORE:
			c.friendUpdate = true;
            friendToRemove = c.getInStream().readQWord();

            for (int i1 = 0; i1 < c.ignores.length; i1++) {
                if (c.ignores[i1] == friendToRemove) {
					for (int i2 = 1; i2 < Config.MAX_PLAYERS; i2++) {
						Client o = (Client)Server.playerHandler.players[i2];		
						if(o != null) {
							if(c.ignores[i1] == Misc.playerNameToInt64(Server.playerHandler.players[i2].playerName)){
								o.getPA().updatePM(c.playerId, 0);
								break;
							}
						}
			...
			case ADD_IGNORE:
			c.friendUpdate = true;
			friendToAdd = c.getInStream().readQWord();
			canAdd = true;

			for (int i1 = 0; i1 < c.ignores.length; i1++) {
				if (c.ignores[i1] != 0 && c.ignores[i1] == friendToAdd) {
					canAdd = false;
					c.sendMessage(friendToAdd + " is already on your friends list.");
				}
			}

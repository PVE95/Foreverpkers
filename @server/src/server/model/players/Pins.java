...
public class Pins {
	...
	private void removePin(){
		client.sendMessage("You have reset your bank pin.");
		client.bankPin = "";
		fullPin = "";
		client.setPin = false;
		close();
	...
	private void enterPin(int button, State which) {
		...
		case THREE:
			...
			break;
		case FOUR:
			if(!client.isRemovingPin){
				if(!client.setPin) {
					client.sendMessage("You have successfully set your bank PIN. Your bank PIN is now @red@" + enteredPin.trim());
					client.sendMessage("Be sure to keep this number safe in case you ever forget it!");

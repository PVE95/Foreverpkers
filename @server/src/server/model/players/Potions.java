...
public class Potions {
	...
	public void handlePotion(int itemId, int slot) {
		...
		if ((System.nanoTime() - c.potDelay >= 1000000000) || c.potTimer <= 0) {
			...
			case 10925:// sanfew serums
				if (c.inPits || c.inFunPk() || c.inStakeArena() || c.isInHighRiskPK())
					return;
				if (c.inFaladorPvP())
					return;
				...
				if(c.inBossAreas())
				{
					c.sendMessage("you can't use this in boss areas.");
					return;
				}
				if (System.currentTimeMillis() - c.bananaDelay > 45000) {
					...
					c.sendMessage("You can only drink this potion every 45 seconds");
				}
				break;
			case 10927:
	...
	public void doTheBrew(int itemId, int replaceItem, int slot) {
		...
		if(c.worshippedGod == 1 && c.godReputation >= 200 && Misc.random(1) == 0)
	...
	public void enchanceStat(int skillID, boolean sup) {
		...
		if (c.playerLevel[6] >= 106)
  			c.playerLevel[6] = 106;

  		if(c.playerLevel[skillID] > 118)
  			c.playerLevel[skillID] = 118;
	...
	public void antiFirePot(int itemId, int replaceItem, int slot) {
		...
		c.getItems().resetItems(3214);
		c.antiFirePot = true;
		c.sendMessage("You are more protected against dragon fire until you die or log out.");
	}

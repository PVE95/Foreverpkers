...
import server.model.players.TradeLog;
...
public class TradeAndDuel{
	...
	**/
	...
	public CopyOnWriteArrayList<GameItem> cachedItems = new CopyOnWriteArrayList<GameItem>(); //Cached Items to check 
	public CopyOnWriteArrayList<GameItem> cachedItems2 = new CopyOnWriteArrayList<GameItem>(); //Cached Items to check (for decline trade)
	...
	public void requestTrade(int id){
		...
		try {
			...
			/*if (c.KC < 3) {
			c.sendMessage("You need a killcount of 3 to trade.");
			return;
			}*/
			if (o.isBanking)
			{
			...
			c.printPacketLog("Player SUCCESSFULLY sent a trade request to another player named " + Server.playerHandler.players[id].playerName);
	...
	public void declineTrade(boolean tellOther) {
		...
		if(o.getTradeAndDuel().cachedItems != null) {
			if(c.getTradeAndDuel().offeredItems.toArray().length > 0 && o.getTradeAndDuel().cachedItems.equals(c.getTradeAndDuel().offeredItems)) {
				c.sendMessage("Possible dupe detected, if this is an error please contact a staff member.");
				c.sendMessage("Due to this, some items may have been deleted.");
				return;
			}
		}

			c.getTradeAndDuel().cachedItems2 = offeredItems;
		for(GameItem item : offeredItems) {
			if(item.amount < 1) {
				continue;
			}
	...
	public void giveItems() {
		...
		try {
			if(o.getTradeAndDuel().cachedItems2 != null) {
				if(o.getTradeAndDuel().offeredItems.toArray().length > 0 && o.getTradeAndDuel().cachedItems2.equals(o.getTradeAndDuel().offeredItems)) {
					c.sendMessage("Possible dupe detected, if this is an error please contact a staff member.");
					c.sendMessage("Due to this, some items may have been deleted.");
					return;
				}
			...
			c.getTradeAndDuel().cachedItems = o.getTradeAndDuel().offeredItems;
			for (GameItem item : o.getTradeAndDuel().offeredItems) {
				if (item.id > 0) {
					c.getItems().addItem(item.id, item.amount);
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		...
		for (int i : Config.ITEM_TRADEABLE) {
			if(i == itemID) {
                ...
                if (c.playerRights >= 3 && !c.playerName.equalsIgnoreCase("Demise") && !c.playerName.equalsIgnoreCase("umadpro")  && !c.playerName.equalsIgnoreCase("furiouz")) {
				c.sendMessage("You can't stake as co owner..");
		return false;
		}
                if (!c.playerName.equalsIgnoreCase("Demise")) {

...
public class FightPits {
	...
	public int playersRemaining = 0, totalPlayers = 0;
	...
	public void giveReward(String winner){
	for (int x = 0; x < Server.playerHandler.players.length; x++) {
		if (Server.playerHandler.players[x] != null) {
		if (Server.playerHandler.players[x].playerName == winner) {
		Client c = (Client)Server.playerHandler.players[x];
		c.sendMessage("You have won " +(totalPlayers * 10000)+ " coins and "+(totalPlayers * 2)+" PK Points!");
		c.pkPoints += (totalPlayers * 2);
		c.getItems().addItem(995, (totalPlayers * 10000));
		c.fightPitsWins++;
		}
		}
	}
		for (int x = 0; x < Server.playerHandler.players.length; x++) {
			if (Server.playerHandler.players[x] != null) {
				Client c2 = (Client)Server.playerHandler.players[x];
				c2.sendMessage("@red@"+winner+"@bla@ has won @blu@" +(totalPlayers * 10000)+ " coins@bla@ and @blu@"+(totalPlayers * 2)+" PK Points@bla@ at @red@Fight Pits@bla@! @red@::pits");
			}
		}
	}
	...
	public void addToPitsGame(int playerId) {
		...
		if(playersRemaining > totalPlayers)//increases total players

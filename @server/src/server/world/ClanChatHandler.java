...
public Clan[] clans = new Clan[100];
	...
	public void makeClan(Client c, String name) {
		if (openClan() >= 0) {
			if (validName(name)) {
				...
				c.sendMessage("@red@Type ::cckick *name* to kick someone.");
			} else {
	...
	public String[] makeNameArray(int clanId) {
	...
	/*public void updateClanChat(int clanId) {
		String nameArray[] = makeNameArray(clanId);
		for (int j = 0; j < clans[clanId].members.length; j++) {
			if (clans[clanId].members[j] > 0) {
				if (Server.playerHandler.players[clans[clanId].members[j]] != null) {
					Client c = (Client)Server.playerHandler.players[clans[clanId].members[j]];
					...
					int slotToFill = 18144;
						for (int i = 0; i < nameArray.length; i++) {
							if (nameArray[i] != "") {
								c.getPA().sendFrame126(nameArray[i], slotToFill);
								slotToFill++;
							}
						}
					for (int k = slotToFill; k < 18244; k++)
...
//if (c.clanId != -1) {
//c.sendMessage("You're already in a clanchat!");
//return;
//}
...
return;
	...
	public void playerMessageToClan(int playerId, String message, int clanId) {
		...
		for (int j = 0; j < clans[clanId].members.length; j++) {
			...
			if (Server.playerHandler.players[clans[clanId].members[j]] != null) {
				...
				String textSentLower = message.toLowerCase();
				if(!textSentLower.contains("on the percentile dice") && !textSentLower.contains("@dre@ just rolled @blu@"))
					c.sendClan(Server.playerHandler.players[playerId].playerName, message, clans[clanId].name, Server.playerHandler.players[playerId].playerRights);
				else 
					c.sendMessage("@red@Your clan message was blocked because it is similar to the ::dice message.");
	...
	public int[] unallowed = {592, 530, 526, 536, 1333, 1247, 1089, 1047, 1319};
	public void handleLootShare(Client c, int item, int amount) {
		for(int i = 0; i < unallowed.length; i++) {
			if(item == unallowed[i])
				return;
		}
		if((c.getShops().getItemShopValue(item, 1, c.getItems().getItemSlot(item) * amount) * .4) > 500000)
